\TODO{articles about viewing interface, getting user input, smart sketchpad image, end 3.6.1}
...
It favors support vector machines as the best performance alternative.
The algorithm itself is relatively simple and may be generalized to 3D shapes.

The method proposed in \cite{FREEDOM01} works nicely but is applied only in 2D space.
Additionally, users found the automatic replacement of geometric shapes as losing control of the application.


The method proposed in \cite{DIGCLAY00} shouldn't be used since it only allows describing part
of geometry (the occluded geometry is undefined) and since it's a discrete algorithm it can't
be reapplied.
...
\begin{figure}[!ht]
	...
	\caption{Pegasus: A diagram drawn on Pegasus without using any editing commands such as rotation, copy or griding (left); interaction with multiple candidates (right).}
	\label{FIG-PEGASUS}
\end{figure}
