...
\begin{description}
	\item[HMD] --
		Head mounted display has the benefits of sticking to the user's head, detecting head tilt.
		On the other hand it serves one single user, most users report suffering
		from fatigue after long periods of usage and it has limited resolution.
	\item[CAVE] --
		Shares the benefit of enclosing the user viewing area with HMDs.
		Has a better resolution though. The downside is the small number of simultaneous users who
		can experience the CAVE simultaneously.
	\item[Wall] --
		Its size and resolution depends entirely on the setup, but normally a wall offers
		high resolution (depends on the number of projectors in the grid and their resolution).
...
Using a Wall or CAVE presents other challenges: synchronization of
the screens, viewport cropping, etc. Several systems exist which deliver
high performance 3D graphics and deliver these features. Based on
scene graphs there are OpenSceneGraph and OpenSG and others.

The last problem in this category is how to effectively render city
landscape. One has to limit the detail of further away objects. Ideally
the transition should be smooth but equally relevant is recognizing
each building's main shape even far away. This is achieved implementing
