...
availabe today on Urban Sketcher, as with the series of User Tests which took place both in
...
The Lisbon tests took place between the 11th and 12th of June 2007 and focused on testing collaboration tasks.
For these tests the Instituto Superior Técnico Multimedia Lab was used\cite{LEME}. It has a structure of 4 by 3 projectors
behind a translucent screen. The projection is performed from the back and 2 infrared cameras are mounted on the
projector structure so a bigger portion of the screen is tracked.
...
\TODO{INSERT PIC FROM GLASGOW, MARKERS AND TRACKING FIGURES}
...
The distributed documents for task performance and questionnaires can be found at APPENDIX\TODO{APP}.
...
This might have to do with users requiring more context menu invocations and temporarily forgetting the correct
gesture to perform or drawing triangles it more roughly.
...
Few users were used to handling laser pointers and some of them were used to make use of vertical boards
...
Due to the slow capturing frequency, strokes have to be drawn slowly, otherwise they get segmented.
This problem is enphasized when performing apply-to-scene operations such as recentering the examine center of interest or instancing new shapes.
...
when compared to Google SketchUp (81\% of the users).
13\% of the users cited the novel interface unfamiliar therefore harder to master for a novice user while only 6\% elected
Google SketchUp as the best interface.
94\% of the users enjoyed the capacity of invoking menus as needed -- of these, 13\% said that the navigation menus
could be fixed on the screen due to their constant usage, though this screen allocation problem being result of the
...
have shown low confidence when asked for perform the modeling tasks, ending up completing them with success and
reasserting their beliefs regarding such tasks.
